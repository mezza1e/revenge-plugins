(function(vendettaArg) {
     ...
     * @description nicola02nb BypassBlockedOrIgnored mobile port. Bypasses the blocked or ignored confirmation warning modal when joining voice channels.
     * @version 1.1.0
    ...
    function notifyActive() {
        try {
            if (_revenge.discord?.actions?.ToastActionCreators?.open) {
                _revenge.discord.actions.ToastActionCreators.open({
                    key: 'plugin-active',
                    content: 'Revengecord Plugin Suite v1.1.0: ACTIVE'
                });
                return;
            }
        } catch (_) {}
        try {
            ...
            if (typeof showToast === 'function') {
                showToast('Revengecord Plugin Suite v1.1.0: ACTIVE');
                return;
            }
        } catch (_) {}
        console.log('[Revengecord Plugin Suite v1.1.0] ACTIVE and loaded successfully');
    }
    ...
    function startPlugin() {
        if (isStarted) {
            console.log('[Revengecord Plugin Suite v1.1.0] Already started, ignoring duplicate call.');
            return;
        }
        isStarted = true;

        try {
            console.log('[Revengecord Plugin Suite v1.1.0] Initializing...');
            ...
            notifyActive();
            console.log('[Revengecord Plugin Suite v1.1.0] Loaded and active successfully.');
        } catch (e) {
            console.error('[Revengecord Plugin Suite v1.1.0] Error:', e);
        }
    }
    ...
    function stopPlugin() {
        ...
        try {
            console.log('[Revengecord Plugin Suite v1.1.0] Stopping...');
            while (unpatches.length > 0) {
                const unpatch = unpatches.pop();
                try { if (typeof unpatch === 'function') unpatch(); } catch (_) {}
            }
            console.log('[Revengecord Plugin Suite v1.1.0] Stopped cleanly.');
        } catch (e) {
            console.error('[Revengecord Plugin Suite v1.1.0] Error stopping:', e);
        }
    }
