 ...
 * @description Hides Ignored and Blocked Users sections from all Settings tabs and Friends tablist.
 * @version 1.3.0
...
(function(vendettaArg) {
    ...
    function notifyActive() {
        try {
            if (_revenge.discord?.actions?.ToastActionCreators?.open) {
                _revenge.discord.actions.ToastActionCreators.open({
                    key: 'plugin-active',
                    content: 'HideBlockedInSettings v1.3.0: ACTIVE'
                });
                return;
            }
        } catch (_) {}
        try {
            ...
            if (typeof showToast === 'function') {
                showToast('HideBlockedInSettings v1.3.0: ACTIVE');
                return;
            }
        } catch (_) {}
    }
    ...
    function startPlugin() {
        try {
            console.log('[HideBlockedInSettings v1.3.0] Initializing...');
            ...
            notifyActive();
            console.log('[HideBlockedInSettings v1.3.0] Loaded and active successfully.');
        } catch (e) {
            console.error('[HideBlockedInSettings v1.3.0 Error]', e);
        }
    }

    function stopPlugin() {
        try {
            console.log('[HideBlockedInSettings v1.3.0] Stopping...');
            while (unpatches.length > 0) {
                const unpatch = unpatches.pop();
                try { if (typeof unpatch === 'function') unpatch(); } catch (_) {}
            }
            console.log('[HideBlockedInSettings v1.3.0] Stopped cleanly.');
        } catch (e) {
            console.error('[HideBlockedInSettings v1.3.0 Error stopping]', e);
        }
    }
